...
apiRoutes.get("/tradepoints", (req, res) => {
  ...
  if (role == 0) {
    tradepoints.aggregate([{$match : {"city":city}}, {$project : {"_id":false, "tp":false, "name":false}}, {$group : { _id:{wp:"$wp", tradepoint:"$tradepoint", address:"$address", city:"$city"}, wp:"$wp", tradepoint:"$tradepoint", address:"$address", city:"$city" }}]).toArray((err, docs) => {
      if(err) { res.sendStatus(400); }
