...
module.exports = function ($rootScope, $scope, $state, dataSource, Gravatar) {
  ...
  $scope.atWork = function(){
    ...
    dataSource.set('/auth/user/atwork', obj).then(function(){
  ...
  $scope.SavePoints = function(point){
    ...
    dataSource.set('/auth/user/tradepoint', obj).then(function(res){
      if (res.data.success) {
  ...
  $scope.Filter = function(){
    dataSource.get('/auth/tradepoints', {city: $rootScope.user.city}).then(function(res) {
